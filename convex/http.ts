...
// ---------------------------------------------------------------------------
// Sign in with Apple (web) — Apple no permite localhost como Return URL.
// El Services ID apunta a este host (EXPO_PUBLIC_CONVEX_SITE_URL).
// ---------------------------------------------------------------------------

function parseAppleState(raw: string | null): { o: string; n?: string } | null {
    if (!raw) return null;
    try {
        const json = atob(raw);
        const parsed = JSON.parse(json) as { o?: string; n?: string };
        if (!parsed?.o || typeof parsed.o !== "string") return null;
        return { o: parsed.o, n: parsed.n };
    } catch {
        return null;
    }
}

function appleAuthCallbackHtml(args: {
    appOrigin: string;
    idToken?: string;
    userJson?: string;
    error?: string;
    ...
    const payload = {
        source: "ramgos-apple-auth",
        id_token: args.idToken ?? null,
        user: args.userJson ?? null,
        error: args.error ?? null,
    };
    return `<!DOCTYPE html>
<html lang="es">
<head><meta charset="utf-8"/><title>Ramgos · Apple</title></head>
<body>
<p style="font-family:system-ui;text-align:center;margin-top:3rem">Completando inicio con Apple…</p>
<script>
(function () {
  var data = ${JSON.stringify(payload)};
  var origin = ${JSON.stringify(args.appOrigin)};
  try {
  ...
  try {
    window.location.replace(origin + "/#ramgos_apple_auth=" + encodeURIComponent(JSON.stringify(data)));
  } catch (e2) {
    document.body.innerHTML = "<p>Podés cerrar esta ventana y volver a la app.</p>";
  }
})();
</script>
</body>
...
http.route({
    ...
    handler: httpAction(async () => {
        ...
        if (!body) {
            return new Response(
                "Apple domain association no configurado. Seteá APPLE_DOMAIN_ASSOCIATION en Convex.",
    ...
    handler: httpAction(async (_ctx, request) => {
        ...
        try {
            if (contentType.includes("application/x-www-form-urlencoded")) {
                ...
                idToken = String(form.get("id_token") ?? "") || null;
                userJson = String(form.get("user") ?? "") || null;
                stateRaw = String(form.get("state") ?? "") || null;
            ...
            console.error("[Apple callback] parse error:", e?.message ?? e);
            return new Response("Bad Request", { status: 400 });
        }

        const state = parseAppleState(stateRaw);
        const appOrigin =
            state?.o ||
            process.env.APPLE_WEB_APP_ORIGIN ||
            "http://localhost:8081";

        if (error) {
            return new Response(
                appleAuthCallbackHtml({
                    appOrigin,
                    error: `Apple: ${error}`,
                }),
        ...
        if (!idToken) {
            return new Response(
                appleAuthCallbackHtml({
                    appOrigin,
                    error: "Apple no envió id_token. Revisá Services ID y Return URL.",
                }),
                { status: 200, headers: { "Content-Type": "text/html; charset=utf-8" } },
            );
        }
...
http.route({
    ...
    handler: httpAction(async () => {
        return new Response(
            "Este endpoint recibe el POST de Apple (form_post). Abrí Login en la web de Ramgos y usá Continuar con Apple.",
