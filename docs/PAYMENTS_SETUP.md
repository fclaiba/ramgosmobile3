...
| Escrow | La plata queda en el balance de la plataforma (`escrowState = held`). Se libera al confirmar recepción, por admin, por disputa a favor del vendedor, o por cron (productos 10 días, bonos 1 día, eventos +24h, servicios: 24 h después del turno — o 7 días desde `delivered` si se vendieron sin agenda). |
...
| Turnos | Un servicio con agenda reserva el HORARIO antes de cobrar (`internal.agenda.internalReserveAppointmentSlot`), con la misma mutation atómica que el stock: de N compradores simultáneos sobre el mismo turno entra uno. El negocio elige si sus turnos se cobran por adelantado o son solicitudes (`businessSettings.appointmentMode`), y su zona horaria manda para todos. Cancelar con más de 24 h (configurable) reembolsa; dentro de la ventana hace falta `force` de admin. El escrow se libera 24 h después de que el turno terminó. |
