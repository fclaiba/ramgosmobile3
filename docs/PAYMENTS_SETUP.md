...
| Cuentas conectadas | **V2** (`v2.core.accounts.create`): `dashboard: express`, `losses_collector: application`, capacidades `stripe_balance.stripe_transfers` (recibir) **y** `stripe_balance.payouts` (retirar). Onboarding hosted (`v2.core.accountLinks.create`) con retorno a la app. Estado reactivo en `users.stripeConnectCaps[Test]`. |
