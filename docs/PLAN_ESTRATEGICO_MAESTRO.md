...
| **CLI-KYC** | Requerimientos del cliente — verificación por email deja de fallar en silencio; teléfono documentado como no implementado | 🟡 Código listo | 85% | 🔴 **`RESEND_API_KEY` no está en `.env.local`**: hoy el OTP cae al mock de consola y no se envía ningún mail. `check-readiness.js` ya lo detecta. Verificación por SMS fuera de alcance por decisión del usuario | 2026-08-24 |
| **CLI-CAT** | Requerimientos del cliente — categorías Barbería y Gimnasio | ✅ Cerrada | 100% | — | 2026-08-24 |
| **CLI-LIMP** | Requerimientos del cliente — limpieza del flujo publicar/comprar (sin tocar Stripe) | 🟡 Parcial | 60% | Borrado el camino muerto de órdenes y los archivos huérfanos. **Faltan los tests de `listings`/`orders`**: `convex/__tests__/` sigue sin cobertura de comercio | 2026-08-24 |
| **CLI-LEGAL** | Requerimientos del cliente — términos completados (tabla íntegra de R Coins, comisión 10%, cargo de gestión) | 🟡 Código listo | 90% | Falta revisión legal. ⚠️ Inconsistencia a resolver: los términos vigentes dan **7 días** para pedir devolución pero el borrador del cliente dice **10 días hábiles**, que es el mismo plazo que la retención de fondos — una devolución pedida el día 10 correría carrera con la liberación | 2026-08-24 |
| **CLI-STRIPE** | Requerimientos del cliente — Parte 2 completa (comisión 10%, escrow, devoluciones, liquidaciones, Connect, panel financiero, QA en Test Mode) | ⚪ Pendiente | 0% | Diferida explícitamente por el usuario ("lo de stripe dejalo para lo último") | 2026-08-24 |
