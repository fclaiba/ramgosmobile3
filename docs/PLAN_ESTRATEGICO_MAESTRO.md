...
| **INT-AUD** | Integridad transaccional — auditoría (STK/AGD/PAY/BON/TRV) + remediación en 7 hitos (H0-H7) | 🟡 H0-H2 cerrados | 35% | H0 ✅ deployment `ramgos-audit` + 4 tests sin skip (faltan 4 secrets en GitHub para CI). H1 ✅ 3 mutations públicas sin auth (bonos/campañas) → internal + test de regresión; **hallazgo nuevo sin resolver**: escalación de privilegios en `syncUser` (`convex/users.ts:702`, crea cuenta con `role` arbitrario), fuera de alcance de H1 a propósito. H2 ✅ BON-07 a nivel 4: refund cancela bono `issued`, bloquea `redeemed` sin `force` de admin, `audit_logs BONO_REFUND_FORCED`. STK-03 y AGD-02 siguen en rojo por diseño (5/5 sobreventa) — se cierran en H3/H4. Siguiente: H3 (reserva atómica, diseño Fable) y H4 (conectar eventos, decidido). Plan: `~/.claude/plans/ac-est-el-prompt-adaptive-zephyr.md` | 2026-09-04 |
