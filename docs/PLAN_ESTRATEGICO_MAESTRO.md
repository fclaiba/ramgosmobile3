...
| **INT-AUD** | Integridad transaccional — auditoría (STK/AGD/PAY/BON/TRV) + remediación en 7 hitos (H0-H7) | 🟡 H0-H3 cerrados | 55% | H0 ✅ deployment `ramgos-audit` + 4 tests sin skip (faltan 4 secrets en GitHub para CI). H1 ✅ 3 mutations públicas sin auth → internal + test de regresión; **hallazgo nuevo sin resolver**: escalación de privilegios en `syncUser` (`convex/users.ts:702`, crea cuenta con `role` arbitrario), fuera de alcance de H1 a propósito — **hoy es el único bloqueante de apertura al público**. H2 ✅ BON-07 a nivel 4. H3 ✅ **STK-01/STK-03 a nivel 4**: tabla `stockReservations` + `convex/stock.ts`; la reserva chequea y descuenta en una sola mutation ANTES de cobrar, el webhook la consume, y el stock vuelve por cuatro caminos (salir del checkout, fallo al crear el PI, `payment_failed`/`canceled`, cron cada 5 min con TTL 30). Se eliminó el pre-check de stock de `internalBuildCheckout` — era media causa raíz y tras H3 daría un falso "sin stock" en cada reintento de pago. AGD-02 queda frenado por `listings.stock` pero **el aforo real sigue desconectado**. Falta re-correr `npm run test:audit` contra `ramgos-audit` (necesita deploy del usuario). Siguiente: **H4** (eventos: `eventCapacity`/`eventSoldCount` + emitir `eventReservations`/QR) | 2026-09-05 |
