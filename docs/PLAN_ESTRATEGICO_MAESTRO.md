...
| **CLI-COMP** | Requerimientos del cliente — flujo de **comprar**: descuento de stock, dirección de envío, máquina de estados, carrito de invitado, back-link de pago, idempotencia del webhook, reconciliación | 🟡 Código listo | 85% | Desplegado a dev; falta prod. 🔴 **Sin QA runtime**: el modo por defecto de la app es simulado (`PaymentModeContext` arranca en `'test'` → `simulate: true` → PI falso), así que **el webhook no se ejercita nunca** y toda la idempotencia (E-130) está sin probar en la práctica. Hay que probar con `mode: 'test'` real de Stripe | 2026-08-25 |
