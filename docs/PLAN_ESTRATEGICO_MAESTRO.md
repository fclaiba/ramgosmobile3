...
| **CLI-STRIPE** | Requerimientos del cliente — Parte 2 completa (comisión 10%, escrow, devoluciones, liquidaciones, Connect, panel financiero, QA en Test Mode) | 🟢 Circuito validado E2E en test | 92% | **Circuito completo ejercido de punta a punta en Stripe test** (E-141): onboarding Connect → checkout → webhook → orden en escrow → liberación con transfer real (`tr_3UBNx4…`, 1983¢ con `source_transaction`) → idempotencia verificada en webhook y en liberación. Los cuatro defectos de dinero que bloqueaban producción están corregidos (orden trabada, punto ciego de reconciliación, modo por defecto + autorización server-side, pantalla que afirmaba éxito sin orden). Webhooks de producción recreados apuntando a `deafening-turtle-227` con la versión de API correcta; los viejos, que apuntaban a dev, deshabilitados. Migración del cursor de reconciliación corrida en prod. **Falta**: compra real de validación, y ejercitar refunds/disputas y el payout a influencer (requiere adelantar `scheduledAtMs`) | 2026-09-03 |
| **CLI-FIN** | Requerimientos del cliente — administración financiera del vendedor (pantalla "Ver saldo y retiros" completa + conexión Stripe reintentable) | 🟢 Código listo (desplegado a dev) | 90% | QA runtime del flujo: cancelar el onboarding en Stripe y confirmar que el botón sigue clickeable ("Continuar onboarding"), completar el KYC y ver el saldo cargar solo, desvincular con saldo > 0 (bloqueado) y con saldo 0 (desvincula). `createExpressLoginLink` degrada a `unsupported`: falta confirmar si el login link V1 acepta cuentas creadas con `v2.core.accounts.create`. Prod pendiente de deploy | 2026-09-04 |
| **INT-AUD** | Integridad transaccional — auditoría (STK/AGD/PAY/BON/TRV), remediación H0-H5 | 🟡 H0-H5 cerrados | 90% | H0 ✅ deployment + tests reales. H1 ✅ seeds sin auth → internal; **hallazgo nuevo sin resolver**: escalación de privilegios en `syncUser` (`convex/users.ts:702`) — **es lo único que queda para abrir al público**, de otra categoría que STK/AGD/PAY/BON/TRV. H2 ✅ BON-07. H3 ✅ STK-01/STK-03 (reserva atómica de stock). H4 ✅ AGD-06/STK-04 (entradas con QR por unidad + guarda de refund). H5 ✅ **AGD-01/02/05/07/08 a nivel 4**: `convex/_agenda.ts` (grilla, zonas horarias con DST vía `Intl`, ventana de cancelación) + tabla `appointments` + `internalReserveAppointmentSlot`, calcado de H3. **Corrección de diagnóstico**: la auditoría decía que la agenda "no está construida"; en realidad estaba enchufada de punta a punta (`FormFillScreen`, `AgendaConfigTab`, `MyBookingsScreen`, `businessSettings`) pero calculaba la grilla EN EL CLIENTE y `submitLead` guardaba cualquier string sin chequear si el horario estaba tomado — dos compradores tomaban el mismo turno. Cierra además el escrow de servicios, que no liberaba NUNCA (esperaba un `delivered` que nadie escribe), y tres bugs adyacentes (catch que tragaba errores, dos full scans, domingo mapeado a 7 vs 0). Falta correr `npm run test:audit` (3 suites nuevas) contra `ramgos-audit` — deploy del usuario | 2026-09-05 |
...
| E-140 | 2026-09-02 | Parte 2 (Stripe) — Webhook | 🔴 **El pago se cobra y del lado del servidor no pasa NADA.** Síntoma reportado: "pasa el pago pero queda en el carrito lo que puse para comprar". El carrito era sólo lo visible | El secreto de firma del webhook de test (`STRIPE_WEBHOOK_SECRET_TEST`, deployment dev) tenía **el mismo valor que el de live** — verificado comparando hashes sin exponer valores. En Stripe, test y live firman con secretos distintos, así que `verifyStripeEvent` fallaba siempre. `convex/http.ts:84-88` devuelve 400 y **retorna ahí mismo**: la línea 99 (`recordPaymentEvent`) nunca se alcanza, así que el rechazo **no deja rastro en la base** — sólo en los logs de Convex y en el dashboard de Stripe. Diagnóstico confirmado con 4/4 indicadores: cero filas en `paymentEvents` para la fecha, `payments.status = "pending"`, `internalGetOrdersForPaymentIntent` → `[]`, y `cart` intacto | Es un fallo de **configuración**, no de código: se corrige cargando el `whsec_` real del destino de test y reenviando el evento desde el Dashboard de Stripe (el backend lo procesa como si recién llegara y recupera la compra huérfana). Lo que el incidente **sí** dejó al descubierto, y queda como deuda: (a) **el vaciado del carrito y TODO el post-pago dependen exclusivamente del webhook** — `internalClearCart` vive en `stripe.ts:767`, dentro de `internalProcessPaidCheckout`, y el cliente nunca lo llama (grep sin resultados de `clearCart`/`useCart` en `src/payments/` y `PaymentScreen.tsx`); sin webhook no se crea la orden, no se descuenta stock, no se debitan los puntos canjeados (el usuario se lleva el descuento **y** los puntos), el pago queda `pending` y el vendedor nunca cobra; (b) **`PaymentScreen` canta éxito con el confirm de Stripe**, sin esperar a que la orden exista: el comprador ve "listo" y se queda sin nada, en silencio; (c) 🔴 **punto ciego de la reconciliación**: la regla `paid_without_order` (`reconciliation.ts:259-311`) filtra por `status === "succeeded" \|\| "succeeded_in_escrow"`, pero un webhook que **nunca llegó** deja el pago en `pending` (`stripe.ts:427`), así que se cuela entre las tres reglas. La red de seguridad cubre "llegó y falló procesando", no "nunca llegó" | ✅ **Resuelto**. Corrección al diagnóstico inicial: no era sólo el secreto mal copiado — **no existía NINGÚN destino de webhook en modo test** (`GET /v1/webhook_endpoints` devolvía vacío), así que Stripe nunca intentó entregar nada. Creados por API los dos destinos apuntando a `https://academic-lapwing-311.convex.site/stripe-webhook-test` (snapshot `we_1UBPeo…` con 15 eventos, y thin V2 `ed_test_61VKndec…`), y cargados sus dos secretos reales en el deployment dev. Las **dos** compras huérfanas (`pi_3UBNx4…`, `pi_3UBOC4…`) se recuperaron replayando sus eventos reales de Stripe con firma HMAC válida — equivalente al *Resend* del Dashboard, y prueba de que el pipeline funciona de punta a punta. Verificado 4/4: `paymentEvents` con los dos eventos `processed`, `payments.status = succeeded_in_escrow` con `orderId`/`stripeChargeId`/`settledAt`, una orden por PI en `escrowState: held`, y el carrito del comprador vacío. El fee real de Stripe se capturó del `balance_transaction`: 117¢ sobre 3000¢ (2.9% + 30¢), correcto. Las tres deudas (a)(b)(c) quedan **abiertas** para retomar después del E2E; (c) es la más seria porque hoy nada detecta un cobro sin orden por webhook ausente | `convex/http.ts`, `convex/stripe.ts`, `convex/reconciliation.ts`, `src/screens/PaymentScreen.tsx` (ninguno modificado todavía — diagnóstico) |
| E-141 | 2026-09-02 | Parte 2 (Stripe) — cierre | 🔴 **Cuatro defectos de dinero encontrados al auditar qué faltaba para habilitar cobros reales**, más la configuración de producción rota. Ninguno se había manifestado porque el circuito nunca se había ejercido de punta a punta | (1) **Orden trabada sin salida**: `confirmReceipt` deja la orden en `release_pending` con `escrowReleaseError: undefined` y agenda la acción; si `internalBeginEscrowRelease` lanzaba (vendedor sin Connect, neto ≤ 0, sin PI) el throw ocurría en el `ctx.runMutation` **fuera del `try`**, así que `internalFlagEscrowReleaseFailed` nunca corría y la orden quedaba en el único estado del que `isReleasable` no deja salir — ni el reintento del comprador ni `adminForceReleaseEscrow` la rescataban. (2) **La reconciliación no veía el modo de falla más común**: `paid_without_order` filtra por `status succeeded*`, pero un webhook que nunca llega deja el pago en `pending`; se colaba entre las tres reglas (la fila existe → no `no_local_payment`; el monto coincide → no `amount_mismatch`). Cero alertas: es E-140, invisible. (3) **Modo por defecto `test` sin autorización real**: con ambos modos configurados ganaba `test`, y eso no es "no cobrar" — el webhook de test crea la orden igual, descuenta stock, otorga puntos y deja al vendedor con un payout respaldado por plata que nunca entró; además `createPaymentIntent` recibía `mode` **como argumento del cliente** sin verificar permisos (el toggle de UI no es una defensa). (4) **La pantalla afirmaba un éxito que podía no haber ocurrido**: confeti apenas Stripe aceptaba la tarjeta, descartando el `paymentIntentId`, sin nada con qué reclamar. **Config**: los destinos de webhook *live* apuntaban a `academic-lapwing-311` (**dev**), con `api_version 2025-02-24.acacia` (el código fija `2026-06-24.dahlia`), el thin sólo con `v2.core.account.updated` de los 3 que maneja el código, y al snapshot le faltaban disputas cerradas, reversals y refunds | (1) inicio de liberación dentro de `try/catch` → el fallo devuelve la orden a `held` con el error visible y reintentable (esa misma mutation queda como herramienta para destrabar). (2) `convex/_reconciliationRules.ts` (módulo puro, 10 tests): regla `charged_but_pending` con período de gracia, más **auto-remediación** — reintenta el handler idempotente antes de alarmar, la misma jugada que reenviar el evento desde el Dashboard, y sólo flaguea si no alcanzó. (3) `src/payments/resolvePaymentMode.ts` (módulo puro, 6 tests): default a `live` cuando está disponible, `STORAGE_KEY` bumpeado para invalidar el `test` guardado en dispositivos, y **autorización server-side** en `createPaymentIntent` (rechaza `test` salvo admin/developer/`isTest`), que es la defensa real. (4) query pública `orders.getCheckoutStatus` (acotada al dueño del pago, reusa el índice `by_stripe_payment_intent`) + estado "Confirmando tu pago…" reactivo en `PaymentScreen`; confeti sólo con la orden creada, y a los 30 s da la cara mostrando el identificador. **Config**: creados por API los destinos correctos apuntando a `deafening-turtle-227` (snapshot 16 eventos + thin con los 3 reales, `api_version` correcta), secretos cargados en el Convex de prod, y los viejos **deshabilitados** (no borrados) para poder volver atrás | ✅ **Resuelto y validado E2E contra Stripe test**: idempotencia del webhook (reenvío → sigue 1 orden); **liberación de escrow con transfer real** `tr_3UBNx4A7Fz349bFU05WJBQ8r` por 1983¢ (3000 − 900 comisión de bono 30% − 117 fee real) con `source_transaction` al charge; y doble liberación → un solo transfer. 384/384 tests, ambos typecheck limpios. Migración del cursor de reconciliación corrida en prod (`copied: true`). **Pendiente del usuario**: una compra real de monto chico de punta a punta antes de abrir a clientes | `convex/stripe.ts`, `convex/reconciliation.ts`, `convex/_reconciliationRules.ts`, `convex/orders.ts`, `src/contexts/PaymentModeContext.tsx`, `src/payments/resolvePaymentMode.ts`, `src/screens/PaymentScreen.tsx`, + 3 archivos de tests |
| E-142 | 2026-09-03 | Parte 2 (Stripe) — camino a producción | 🔴 **El código de pagos estaba listo pero no había forma segura de que llegara a los usuarios.** Al preguntar "¿ya funciona en producción?" se auditó el camino de despliegue completo y aparecieron tres agujeros, ninguno en el código de negocio. Contexto tranquilizador verificado antes de tocar nada: en prod la tabla `payments` está **vacía** y `orders` sólo tiene 2 filas de smoke test (feb/mar 2026, sin `stripePaymentIntentId`) — **nunca pasó un pago real por producción**, así que no había dinero en riesgo | (a) **Sin OTA**: `expo-updates` no instalado, `app.json` sin `updates` ni `runtimeVersion`, `eas.json` sin canales. El arreglo de test-vs-live vive en el **cliente**, y con la app publicada en tiendas esos binarios quedan congelados: ningún push los alcanza. (b) **El CI nunca construye producción**: `ci.yml:82` buildea el perfil `preview`, que apunta al Convex de **dev** y sin embargo llevaba `EXPO_PUBLIC_STRIPE_KEY_LIVE`; con el nuevo default a `live` (E-141) ese build interno arrancaba en modo real contra el backend equivocado. El perfil `production` nunca se construye solo y `submit.production` está vacío. (c) **`.vercelignore` no excluía `.env.local`**: teniendo `.vercelignore`, el CLI de Vercel **deja de aplicar `.gitignore`**, así que un `vercel deploy` manual subía ese archivo y `expo export` lo leía, horneando el Convex de **dev** y `STRIPE_MOCK_MODE=true` en el bundle de producción, pisando lo del dashboard. (d) Además, el backend no se había desplegado: el push `ebcce56` no llegó a prod (CI pendiente o fallido) | Backend desplegado a mano (`npx convex deploy`), verificado con `orders:getCheckoutStatus` respondiendo en prod. `.vercelignore` excluye `.env*.local`. `pk_live` sacada de los perfiles `development` y `preview` (**el perfil `production` quedó intacto, con `pk_test` y `pk_live`**): ahora la única build con clave de cobro real es la única que apunta a prod. `expo-updates` instalado + `runtimeVersion` con policy `appVersion` (un update sólo se entrega a binarios con la misma versión nativa, que es lo que evita mandar JS nuevo a un binario sin los módulos que ese JS necesita) + canal por perfil. **La OTA no alcanza a los binarios ya publicados** — sirve de la próxima versión en adelante, que es justo cuando empieza a haber dinero real. De paso `package.json` declara `expo-splash-screen`, que estaba instalado y referenciado en `app.json` pero sin declarar: un `npm ci` limpio fallaba | 🟡 Lo automatizable, cerrado y verificado (lint, 384/384, ambos typecheck). **Camino crítico en manos del usuario**: `eas build --profile production` + publicación en tiendas (marca el calendario, la revisión tarda días), verificar en el dashboard de Vercel que `EXPO_PUBLIC_CONVEX_URL` apunte a `deafening-turtle-227` y que `STRIPE_MOCK_MODE` no esté en `true`, **rotar los secretos** (`.env.local` tiene la `sk_live` en texto plano y además fueron expuestos en el chat de esta sesión al listar env vars), y la compra real de validación. Riesgo anotado: `STRIPE_SECRET_KEY_TEST` sigue en prod por decisión del usuario, así que un binario viejo que llegue a prod elegiría modo test | `.vercelignore`, `eas.json`, `app.json`, `package.json` |
| E-143 | 2026-09-03 | Parte 2 (Stripe) — cierre y documentación | 🟡 **El usuario reportó estar perdido con el estado de los pagos.** La causa no era falta de documentación sino **exceso de documentación contradictoria**: 12 archivos hablaban de pagos, con 8 contradicciones cruzadas. Además quedaban tres flujos que nunca habían corrido contra Stripe | (a) `ARQUITECTURA_STRIPE_CONNECT_SPLIT.md` era **~85% falso**: describía el código anterior al rewrite (`internalProcessMultiVendorCart`, `internalReleasePaymentAction`, `createConnectAccount` — ninguna existe) y presentaba como gaps abiertos cinco cosas ya resueltas (mock silencioso, pago a influencers, stub del webhook V1, `connectV2.ts`, falta de tests). (b) Dos tasas de comisión distintas en circulación (10% real vs 12% en `FIN_DE_TURNO_REPORTE.md`). (c) Tres versiones de los estados de escrow, una con un `release_scheduled` **inventado** (`DOCUMENTACION_ARQUITECTURA.md` §13). (d) Tres URLs de retorno del onboarding, dos de ellas **dentro del mismo documento** (`PAYMENTS_SETUP.md:38` https vs `:113` `ramgos://`). (e) `DEV_COMPLETENESS.md` y `DIAGNOSTICO_APP_ESTADO_ACTUAL.md` afirmaban "Pagos 100% GO". (f) `ARQUITECTURA_ESCROW.md` §1-3 presentaba como implementada una tipología que no existe (tracking de correo, hitos por fases, infoproductos, liberación por QR). (g) `ARQUITECTURA_SOCIAL_COMMERCE.md` §12 seguía prometiendo `processSimulatedPayment` y "pagos simulados", contradiciendo al §7 del mismo documento | **Pruebas E2E de los tres flujos pendientes**: reembolso real `re_3UBOC4A7Fz349bFU1w9BIMXb` (3000¢, orden a `refunded`/`cancelled`, `refundedCents` cuadrado, segundo intento rechazado); ciclo de disputa completo sobre la orden liberada — `charge.dispute.created` la congela guardando `released` como `escrowPrevState`, y `closed/won` la restaura; el cron de payouts de influencer corre limpio. **Documentación**: nuevo `docs/PAGOS.md` como puerta de entrada única (cómo funciona, qué está probado con evidencia, qué falta separando lo mío de lo del usuario, e índice de los otros docs); `ARQUITECTURA_STRIPE_CONNECT_SPLIT.md` movido a `_archive/` con banner de advertencia, rescatando antes sus tres decisiones de arquitectura (SCT vs destination charges, `losses_collector: application`, ausencia de `application_fee_amount`); y corregidas una por una las siete afirmaciones falsas de (b) a (g) | 🟢 **Circuito de pagos cerrado y documentado.** 384/384 tests, ambos typecheck. **Único tramo sin ejercitar**: la transferencia real a un influencer — requiere una compra hecha con código de referido, y las de prueba se hicieron sin uno (`influencerCents: 0`, por eso no hay fila `payouts` que pagar). La disputa se probó con firma y PaymentIntent reales pero objeto `dispute` sintético: Stripe no deja fabricar disputas sobre cargos existentes. Ambas limitaciones quedan escritas en `PAGOS.md` §3 | `docs/PAGOS.md` (nuevo), `_archive/ARQUITECTURA_STRIPE_CONNECT_SPLIT_2026-09-01.md`, `docs/{PAYMENTS_SETUP,ARQUITECTURA_ESCROW,ARQUITECTURA_SOCIAL_COMMERCE,DOCUMENTACION_ARQUITECTURA,DEV_COMPLETENESS,DIAGNOSTICO_APP_ESTADO_ACTUAL,FIN_DE_TURNO_REPORTE}.md` |
| E-144 | 2026-09-03 | Parte 2 (Stripe) — activar live | 🔴 **Caída activa causada por E-141: ningún usuario podía pagar.** `getPublicConfig` devolvía `{test:true, live:true}` a todo el mundo; el cliente publicado, con ambos modos disponibles, elige `test`; y el gate de E-141 rechaza `test` para quien no es admin. El checkout tiraba "El modo de prueba está restringido" a todos | El modo llega como argumento del cliente, así que la UI nunca fue una defensa — pero la corrección de E-141 se hizo sólo del lado del cliente (default a `live`) más un gate que rechaza, y los binarios publicados no reciben cambios de cliente (sin OTA). Faltaba que el **servidor** fuera la autoridad sobre qué modo le corresponde a cada quien | `convex/_paymentModeAccess.ts` (módulo puro, 10 tests): `getPublicConfig` pasa a reportar la disponibilidad de `test` **por actor**. Al comprador común le responde `{test:false, live:true}` y **el binario ya publicado, con su propia lógica y sin cambiarle una línea, calcula `availableModes=['live']` y elige live solo** — cobra de verdad y la UI muestra "live", así que tampoco se le miente a quien pone la tarjeta. Admin/developer/isTest, pasando `sessionToken`, conservan el toggle. `sessionToken` es `v.optional` **a propósito**: los clientes publicados llaman con `{}` y volverlo obligatorio los dejaría inutilizables por error de validación. El gate se mantiene **rechazando** y no coercionando (cobrar real con la UI diciendo "prueba" es peor que un error), con mensaje accionable. Además: `AuthContext` deja de publicar `undefined` durante `loading` — sin eso el cliente nuevo oscilaba en bucle, porque `App.tsx` remonta el árbol por `key` al cambiar la clave de Stripe; y se unificó el predicado en 3 pantallas que se olvidaban de `developer`. **Riesgo bloqueante descartado con evidencia**: se inspeccionó el bundle web publicado y tiene `pk_live` embebida, así que `availableModes` no queda vacío y `StripeKeyGate` no bloquea la app | ✅ Resuelto y verificado en los dos deployments: sin token → `{test:false, live:true}`; con token de admin real → ambos. 394/394 tests, ambos typecheck | `convex/_paymentModeAccess.ts`, `convex/stripe.ts`, `src/contexts/{PaymentModeContext,AuthContext}.tsx`, `src/payments/components/PaymentModeToggle.tsx`, `src/screens/{CartScreen,SettingsScreen}.tsx` |
| E-145 | 2026-09-03 | Parte 2 (Stripe) — infraestructura | 🔴 **Los cobros reales desde la web se habrían cobrado sin crear la orden.** Detectado al preguntar el usuario "¿no puedo cobrar dinero real desde la web?" — una pregunta que expuso un supuesto equivocado de toda la sesión | **El deployment llamado "dev" (`academic-lapwing-311`) ES producción en los hechos**: la web publicada apunta ahí (verificado inspeccionando el bundle de `ramgos.app`), ahí están los usuarios y las órdenes, y tiene cargada `sk_live`. El llamado "producción" (`deafening-turtle-227`) está vacío — nunca pasó un pago. Nunca se hizo un build de producción en EAS (los únicos son `development` y `preview`, ambos apuntando también al primero). En E-142 se recrearon los webhooks live apuntando a `deafening-turtle-227` **y se deshabilitó el que apuntaba a donde realmente vive la web**, creando la ruptura: el pago se registra en un deployment y el aviso de Stripe llega a otro → cobro real sin orden | Creados los destinos live (snapshot 16 eventos + thin V2, `api_version` correcta) apuntando a `academic-lapwing-311`, y cargados sus secretos ahí. Verificado de punta a punta enviando un evento live firmado al endpoint: responde 200, o sea la firma valida y el webhook procesa. Los destinos hacia `deafening-turtle-227` quedan activos por si algún día se migra. Documentado en `docs/PAGOS.md` §1-bis como advertencia destacada, porque es el supuesto que hay que tener presente antes de tocar cualquier configuración | ✅ Resuelto. **Deuda arquitectónica anotada**: ordenar esto (mover la web a `deafening-turtle-227`) implica migrar todos los datos; mientras no se haga, `academic-lapwing-311` debe tratarse como producción | `docs/PAGOS.md` + configuración de Stripe y env vars (sin cambios de código) |
| E-146 | 2026-09-03 | Parte 2 (Stripe) — deuda de código | 🔴 **Cinco caminos donde la plata se trababa o se perdía**, encontrados al auditar qué quedaba a nivel código con el sistema ya cobrando en live. El más grave no estaba en ninguna lista previa | **A1 (crítico)**: el defecto de E-141 **seguía vivo en el camino del reembolso**. `disputes.ts:322` pre-seteaba `refund_pending` con `escrowRefundError: undefined` y `stripe.ts` llamaba a `internalBeginOrderRefund` **fuera del `try`**; como `isRefundable` rechaza ese estado, lanzaba, `internalFlagOrderRefundFailed` nunca corría, y la orden quedaba en el único estado del que no se puede salir. **El comprador ganaba la disputa y nunca cobraba.** **A5**: la reconciliación leía 100 movimientos y paraba — el comentario decía que paginaba pero `has_more` se ignoraba; con ~30-40 órdenes diarias (cada cobro genera 2 movimientos y cada liberación 1 más) el cursor se atrasaba de forma acumulativa e irreversible, **reabriendo por volumen el punto ciego que E-140/E-141 taparon**. Además `lastSeenId` se asignaba al ENTRAR al loop, así que un movimiento que fallaba a mitad quedaba marcado como visto sin revisarse. **A2**: el payout al influencer sin cuenta Connect iba derecho a `failed`, terminal porque el cron sólo levanta `scheduled` → comisión perdida en silencio. **A3**: una orden cuya liberación falló quedaba fuera de los **tres** crons para siempre. **A4**: la clave de idempotencia del refund dependía del contador de intentos, que avanza aunque el intento falle → un refund parcial cuya reversión fallaba generaba, al reintentar, un **segundo refund y una segunda reversión**, descontándole dos veces al vendedor. **B1/B2**: los flags de reconciliación no notificaban a nadie y la fila de admin no mostraba el modo ni los ids relacionados, que sí se persistían | A1 arreglado **en los dos lados** (no pre-setear + begin dentro del `try`), con tests simétricos a los del release. A5: paginación real con `autoPagingEach` acotada, y `lastSeenId` confirmado en un `finally` (necesario porque el cuerpo tiene 5 `continue`, que son salidas exitosas). A2/A3: reintento con espera creciente (1,2,4… días, techo 7) y tope finito, avisando al influencer y a admins al agotar. A4: clave atada a `refundedCents`, que sólo cambia cuando el refund se completó. B1: aviso **agrupado por corrida**, no por flag. **Hallazgo al verificar**: los flags de fallo revertían el estado a ciegas, así que un intento fallido sobre una orden ya `refunded` la devolvía a `held` — la des-reembolsaba; ahora sólo revierten si la operación estaba en vuelo. **`convex-test` no sirve acá**: requiere Vitest (`import.meta.glob`) y el repo usa Jest — se probó, no arranca, se revirtió; plan B ya previsto: lógica extraída a `convex/_payoutRetry.ts` (módulo puro, 13 tests) | ✅ Resuelto. 415/415 tests, ambos typecheck (el strict volvió a atrapar un narrowing que el de Convex no ve — la trampa de E-136). Desplegado a los dos deployments. **Deuda menor anotada**: la orden de prueba `jd749xa…` quedó con `refundedCents: 6000` sobre 3000 por una llamada de restauración duplicada durante la verificación — dato inconsistente en una orden de test ya cerrada, sin efecto sobre las reglas de reconciliación | `convex/_payoutRetry.ts`, `convex/stripe.ts`, `convex/disputes.ts`, `convex/reconciliation.ts`, `convex/schema.ts`, `convex/orders/_escrowStates.ts`, `src/screens/admin/AdminFinanceScreen.tsx`, + 2 archivos de tests |
| E-147 | 2026-09-03 | Parte 2 (Stripe) — datos | 🔴 **Una cuenta Connect de TEST estaba guardada en el campo de LIVE**, y ese vendedor tiene 4 publicaciones activas: cualquier compra suya en modo real habría cobrado bien y después **trabado la plata** al no poder transferirle. Encontrado revisando el estado real (no el código) tras pedir "fijate qué falta para que ande" | Es la deuda de datos que dejó la migración bi-modal. `users.stripeConnectAccountId` **era** el único campo de cuenta Connect, sin concepto de modo; el rewrite lo redefinió como "específicamente la cuenta LIVE" y agregó `stripeConnectAccountIdTest`, pero **nadie migró los datos existentes** — y como hasta entonces sólo se había operado en test, esas cuentas quedaron etiquetadas como de producción. Estaba anticipado en la primera auditoría de schema de esta sesión ("aditivo-seguro a nivel schema, pero behaviorally significant"): se mergeó el código y se olvidó el dato. Verificado contra Stripe: `acct_1U3Kd4AgTe8R7QSp` da "Permission denied" con la clave live, devuelve `display_name: "oscar ramgos"` con la de test, y no figura entre las 8 cuentas conectadas reales de la plataforma live | `convex/migrations/connectAccountModeFix.ts`: `internalAction` (no mutation — el id de una cuenta no dice a qué modo pertenece, hay que **preguntárselo a Stripe**) que para cada cuenta en el campo live prueba primero con la clave live, después con la de test, y la reubica; las que no existen en ningún modo se limpian para que el vendedor pueda rehacer el onboarding en vez de arrastrar un id que hace fallar cada liberación. Corrida primero en `dryRun`, que confirmó exactamente el diagnóstico. Además `finance.listOrdersWithReleaseError`: hasta ahora una liberación fallida sólo se veía si existía una fila `payouts` en `failed`, y cuando el fallo ocurre al INICIAR la liberación esa fila no llega a existir — la orden quedaba invisible salvo en `listStuckEscrows`, que recién barre a los 30 días | ✅ Resuelto y verificado: 1 cuenta reubicada, la columna `stripeConnectAccountId` **desapareció de la tabla** (ningún usuario la tiene), los otros 3 vendedores intactos, y la migración es idempotente (segunda corrida: 0 cambios). 415/415 tests, ambos typecheck, desplegado a los dos deployments. **Pendiente del usuario**: ese vendedor tiene que rehacer el onboarding en modo real — hoy sus 4 productos se pueden comprar pero no se le puede pagar | `convex/migrations/connectAccountModeFix.ts`, `convex/finance.ts` |
| E-148 | 2026-09-04 | Parte 2 (Stripe) — UI de administración financiera | 🔴 **La conexión de Stripe se podía intentar una sola vez.** Tocabas "Conectar cuenta de pagos", cancelabas en el formulario de Stripe, volvías, y el botón ya no estaba: lo reemplazaba "Ver saldo y retiros", que llevaba a una pantalla cuyo único mensaje era "completá el onboarding desde el panel principal". Loop cerrado, sin salida. Además la pantalla de saldo pintaba **$0.00 cuando fallaba leer el balance**, no tenía reintento ni pull-to-refresh, y mandaba dos veces "al dashboard de Stripe Express" sin ofrecer ningún link | `ensureConnectAccount` (`convex/connect.ts:219`) persiste el `accountId` **antes** de que el usuario complete el formulario de Stripe — es correcto, la cuenta hay que crearla para poder generar el account link. El error fue que las pantallas ramificaban por "¿existe accountId?", que después de ese punto es `true` aunque el KYC no se haya tocado. Ese branching estaba **duplicado a mano** en los dos dashboards (bien en el de negocio, mal en el de influencer): la duplicación ES el bug. Secundarios: `start()` devolvía el resultado del browser como si fuera la verdad (con `returnUrl` https, que Stripe exige, la sesión de auth casi nunca resuelve `success` aunque el onboarding se haya completado), `refresh()` se tragaba los errores, y `Crypto.randomUUID()` se regeneraba **en cada intento** de payout → si Stripe creaba el payout y se cortaba la respuesta, el reintento creaba un segundo | `useConnectOnboarding` expone `phase` (`loading｜unconfigured｜none｜pending｜review｜ready`) como fuente única de branching; `start()` devuelve `ConnectStartResult` y deriva `ready` de releer Stripe, no del browser; `refresh()` devuelve `{ok, status, message}`. `ConnectStatusBanner` (nuevo) concentra el branching en un solo lugar, con `variant=card` que incluye un **"Actualizar estado" permanente** — la salida cuando el onboarding quedó a medias. `WithdrawalScreen` reescrita con estado derivado de 6 ramas (nunca `$0.00` falso: guión + reintento), `Promise.allSettled` para que un fallo del calendario no tape el saldo, historial de retiros, y sección de cuenta (dashboard Express / cambiar / desvincular con guarda de plata). Backend aditivo: `listRecentPayouts`, `unlinkConnectAccount` + `internalClearConnectAccount`, `createExpressLoginLink`. `ConnectReturnScreen` espera a que hidrate la sesión antes de armar el latch, vuelve al `returnTo` guardado a nivel módulo, y el `refresh_url` regenera el link en vez de mostrar un spinner. Idempotencia del payout atada al monto. **Hallazgo al implementar**: el fix de `targetUserId` que el plan proponía habría **roto el dashboard de negocio** — navega con `wallet.ownerId`, que sale del FintechContext mock y puede valer `"business_demo"`, así que `resolveTargetUser` le habría respondido "No autorizado." a cualquier vendedor común; se agregó guarda por rol. Se borraron `finance/WalletScreen` y `marketplace/SellerWalletScreen` (100% mock, cero call sites) | ✅ Resuelto en código. 415/415 tests, typecheck limpio, desplegado a **dev** (`academic-lapwing-311`). **Pendiente**: QA runtime del flujo (el bug es de flujo, no unitario) y deploy a prod | `convex/connect.ts`, `src/hooks/useConnectOnboarding.ts`, `src/hooks/connectReturnTarget.ts`, `src/components/connect/ConnectStatusBanner.tsx`, `src/screens/WithdrawalScreen.tsx`, `src/screens/ConnectReturnScreen.tsx`, `src/screens/InfluencerDashboardScreen.tsx`, `src/screens/BusinessDashboardScreen.tsx`, `App.tsx` |
| E-149 | 2026-09-04 | Integridad transaccional — auditoría | 🔴 Auditoría formal de invariantes (scanner AST `scripts/audit/marketplace-audit.mjs`, 124 archivos, 543 límites transaccionales). **5 críticos**: (1) sobreventa de unidad única por diseño — chequeo en `createPaymentIntent` (action) y descuento en el webhook (mutation), sin reserva; `stockShortfall` se escribe y nadie lo lee, y el cron de auto-release paga la orden sobrevendida igual; (2) el refund no cancela el bono (`bonoRedemptions` intacto; estado `cancelled` existe y nadie lo escribe) → $100 de crédito gratis, lo paga el negocio; (3) `holdEventCapacity` e `internalIssueEventReservationsForPayment` con **0 call sites** — los eventos no controlan aforo ni emiten QR, y el comentario que dice que CheckoutScreen las llama es falso; (4) `seedMockBonos`/`seed5Bonos` mutations públicas sin auth que crean bonos sobre negocios reales; (5) cero tests de concurrencia. Sano: pagos, escrow, refunds, reversiones, idempotencia de webhooks (nivel 3) | Ver `docs/audit/ESTADO-MARKETPLACE.md` §3 | Diagnóstico con evidencia `archivo:línea`, backlog de 12 priorizado por pérdida económica, tests puros ejecutables (7) y de integración (4). **Hallazgo lateral**: `bookings` es una tabla muerta con forma de alquiler; la agenda no existe. Bonos y servicios nacen con `stock: 1` (default del cliente) y el checkout los rechaza desde la segunda venta | ✅ Diagnóstico entregado; remediación abierta (INT-AUD) | `docs/audit/`, `scripts/audit/`, `tests/audit/` |
| E-150 | 2026-09-04 | INT-AUD H0 | Los 4 tests de concurrencia estaban en `skip` por falta de deployment; y al conectarlos, **tres trampas de herramienta** escondían el resultado: `jest-expo` corre en el entorno de React Native cuyo `fetch` es un polyfill sobre XHR mockeado (las N llamadas fallaban en silencio, `r.ok=0`, que se parece a un bug), `createPaymentIntent` exige `expectedTotalCents` exacto y `lineItems` reales, y **`npx convex dev` reescribe `.env.local`** (CONVEX_DEPLOYMENT y EXPO_PUBLIC_CONVEX_URL) aunque se le pase `--env-file` o la variable por entorno — dos veces dejó la app local apuntando al deployment vacío | Sin deployment de pruebas; `convex-test` no corre con Jest (E-146) | Proyecto aparte `ramgos-audit` (`oceanic-goose-862`) con sólo claves TEST y `AUDIT_FIXTURES=true`; `convex/audit/fixtures.ts` (`internalMutation` con guarda doble, marcador `audit-fixture`, seed/inspect/reset); `jest.audit.config.js` en Node puro + `npm run test:audit`; `_fixture.ts` calcula el total con el mismo `internalBuildCheckout` del servidor; `.env.audit` ignorado; backup+diff de `.env.local` alrededor de cada `convex dev` | ✅ 4 tests reales sin skip: **STK-03 y AGD-02 en rojo (5/5 sobreventa, confirmado empíricamente)**, BON-01 y PAY-01 verdes → nivel 4. Job `audit-concurrency` en CI a la espera de 4 secrets. `npm test` 422 verdes, typecheck limpio | `convex/audit/fixtures.ts`, `tests/audit/`, `jest.audit.config.js`, `jest.config.js`, `package.json`, `.github/workflows/ci.yml`, `.gitignore` |
| E-151 | 2026-09-04 | INT-AUD H1 | Barrido TRV-01 con el scanner en modo estricto (`--strict-seeds`, nuevo): `seedMockBonos`, `seed5Bonos` y un **tercer hallazgo del mismo patrón encontrado al barrer** — `convex/campaigns.ts:746 seedBusinessInviteInfluencer1` (activa una campaña influencer/negocio con patch de rol, mutation pública sin auth) — eran invocables por cualquier cliente con la URL del deployment. El scanner por defecto exime toda funcion "seed*" de TRV-01 (para no saturar el reporte narrado a un humano): exactamente el punto ciego que dejó pasar el bug original, porque el hallazgo fue manual, no automático. Al endurecer la deteccion tambien aparecieron 11 falsos positivos en el modulo social (`assertSocialActor` llama a `requireActor` por dentro; el scanner solo reconocia el nombre literal) | Exención por nombre en el diseño del scanner + deteccion de auth por nombre literal de funcion, no por wrapper indirecto | Las 3 mutations → `internalMutation`. Scanner: flags `--strict-seeds` (saca la exencion) y `--out-dir` (no pisa el reporte versionado); `AUTH_WRAPPER_CALLS` reconoce `assertSocialActor`/`resolveTargetUser`. Test `convex/__tests__/publicWriteAuth.test.ts`: corre el scanner estricto y falla si una mutation/action publica sin auth escribe una tabla o campo sensible, con un allowlist explicito y revisado de fronteras de auth legitimas (register/login/logout/reset por OTP/OAuth) | ✅ Resuelto y en CI (`npm test`). **Hallazgo nuevo, NO resuelto, documentado y excluido explicitamente (no silenciado)**: `syncUser` (`convex/users.ts:702-759`) crea una cuenta nueva con `role: args.role as any` sin validar contra el union y devuelve un `sessionToken` ya autenticado — un caller sin sesion puede pedir `role:"admin"`. Es una escalacion de privilegios real, de otra categoria que E-149, y toca el nucleo de auth: necesita su propio hito. 424/424 tests, typecheck limpio (tsc + convex) | `convex/bonos.ts`, `convex/developer.ts`, `convex/campaigns.ts`, `scripts/audit/marketplace-audit.mjs`, `convex/__tests__/publicWriteAuth.test.ts` |
| E-152 | 2026-09-04 | INT-AUD H2 | El refund no cancelaba `bonoRedemptions`: un bono `issued` (nunca canjeado) sobrevivia al reembolso, y uno `redeemed` (credito ya entregado por el negocio en su local) tambien -- el comprador recuperaba la plata Y se quedaba con el credito gastado, perdida que paga el negocio, no la plataforma (BON-07, E-149) | `internalCompleteOrderRefund` nunca leia `bonoRedemptions`; la tabla no tenia indice `by_order` para hacerlo sin leer de mas | `internalBeginOrderRefund` (misma mutation que pone `refund_pending`) lee los bonos de la orden por el indice nuevo `by_order` -- read set acotado a proposito, nada de `by_owner`/`by_listing` que conflictuarian con otro canje del mismo comprador o listing. Bono `issued` se cancela siempre; `redeemed` bloquea el refund salvo `force` explicito de un admin (`adminRefundEscrow`), que ademas deja `audit_logs BONO_REFUND_FORCED`. `force`/`externalRefund` (disputa perdida, refund desde el dashboard de Stripe -- la plata ya se movio afuera, no es negociable) tambien lo saltean. **No se toco** `_escrowStates.ts`: `isRefundable` sigue permitiendo `released`/`held` para todo tipo de listing a proposito, documentado con un comentario -- la maquina de estados del dinero no debe saber de bonos | ✅ Resuelto y verificado con 3 tests de integracion reales contra `ramgos-audit` (issued→cancela solo, redeemed sin force→bloquea, redeemed con force→pasa y deja auditoria), los 3 en verde. 424/424 tests, typecheck limpio | `convex/schema.ts`, `convex/stripe.ts`, `convex/_audit.ts`, `convex/orders/_escrowStates.ts`, `convex/audit/fixtures.ts`, `tests/audit/bonoRefund.integration.test.ts` |
| E-153 | 2026-09-05 | INT-AUD H3 | Sobreventa de unidad única por diseño (STK-01/STK-03, E-149): el chequeo de stock vivía en `createPaymentIntent` (action, antes de cobrar) y el descuento en el webhook (mutation, después de cobrar). 5 compradores simultáneos sobre stock 1 pagaban los 5 — verificado empíricamente 5/5 en H0, no inferido — y el cron de auto-release les pagaba al vendedor las 5 órdenes | El invariante "una sola orden pagada por unidad" no vivía en ninguna transacción: una query no puede reservar, y entre la query y la mutation del webhook estaba el pago entero | Tabla `stockReservations` (índices `by_cart_and_user`, `by_payment_intent`, `by_status_and_expires`) + `convex/stock.ts`. `internalReserveStock` chequea y descuenta en UNA mutation (serializable por OCC) desde `createPaymentIntent`, antes de `paymentIntents.create`; idempotente por `(cartId, userId)`, el mismo par de la idempotency key del PI. El webhook consume la reserva en la misma transacción que crea las órdenes y no vuelve a descontar (`remainingToDecrement`). Devolución por cuatro caminos: `stock.releaseMyCheckoutReservation` desde el unmount de `PaymentScreen`, compensación en el `catch` de `createPaymentIntent`, `payment_intent.payment_failed`/`canceled`, y el cron `expire-stock-reservations` (5 min, TTL 30 min). Aritmética en el módulo puro `_inventory.ts` (`planReservation` todo-o-nada, `remainingToDecrement`). **Dos defectos del propio diseño encontrados en la revisión y corregidos antes de commitear**: (1) `PaymentScreen` genera un `cartId` nuevo en cada montaje y `handlePay` llama a `createPaymentIntent` en cada intento, así que el pre-check de `internalBuildCheckout` le habría dicho "sin stock" al comprador por su PROPIA reserva tras una tarjeta rechazada → se eliminó ese pre-check, la reserva es la única autoridad; (2) el `catch` compensatorio liberaba también una reserva `reused` que pertenecía a un intento anterior todavía pagable → sólo libera las que creó él | ✅ Resuelto. 438/438 tests (14 nuevos: `planReservation`, `remainingToDecrement`, y la falsación pura reescrita con `checkoutRaceReserved` + `checkoutRaceLegacy` como regresión), typecheck 0 errores. **Pendiente de deploy**: re-correr `npm run test:audit` contra `ramgos-audit` para la confirmación empírica de STK-03 | `convex/stock.ts`, `convex/schema.ts`, `convex/stripe.ts`, `convex/http.ts`, `convex/crons.ts`, `convex/_inventory.ts`, `src/screens/PaymentScreen.tsx`, `tests/audit/`, `docs/audit/ESTADO-MARKETPLACE.md` |
| E-154 | 2026-09-05 | INT-AUD H4 | Eventos no controlaban aforo ni emitían entradas (AGD-06/STK-04, E-149): `holdEventCapacity`/`releaseEventCapacity` (aforo vía `eventCapacity`) e `internalIssueEventReservationsForPayment` (emisión de QR) existían, bien escritas, con 0 call sites — la primera porque ninguna pantalla escribe `eventCapacity` (`CreateListingScreen` no tiene ese campo), la segunda porque estaba diseñada para un checkout de un ítem por pago que el rewrite de Stripe Connect dejó atrás | El comentario de cabecera de `events.ts` afirmaba que "CheckoutScreen llama a holdEventCapacity": nunca fue cierto, la misma clase de comentario engañoso que ocultó STK-01 | Se ELIMINARON las tres funciones muertas en vez de conectarlas: investigar mostró que `eventCapacity`/`eventSoldCount` no son una fuente de aforo paralela sin usar, son schema sin escritor. La reserva de H3 ya cubre el aforo real (un evento con `stock:1` se comporta como un producto). Nueva `internalIssueEventReservationsForOrder` (mismo patrón que `bonos.internalIssueBonosForOrder`): una fila `eventReservations` con QR por unidad, disparada desde `internalProcessPaidCheckout` junto al bono. Refund: `internalBeginOrderRefund` lee `eventReservations by_order` (índice nuevo) con el criterio de BON-07 — `checked_in` bloquea sin `force` (`audit_logs EVENT_REFUND_FORCED`), `confirmed` cancela siempre; `internalCompleteOrderRefund` extiende el restock de stock a `event`. **Defecto propio encontrado en la revisión**: `internalAutoReleaseEvents` asumía una fila por orden (cierto antes de H4); con una fila por unidad, una compra de 3 entradas habría programado el release 3 veces para la misma orden → se agregó deduplicación por `orderId` antes de commitear | ✅ Resuelto. 438/438 tests, typecheck 0 errores. 3 tests de integración nuevos (`tests/audit/eventRefund.integration.test.ts`, mismo molde que H2) pendientes de correr contra `ramgos-audit` (deploy del usuario) | `convex/events.ts`, `convex/schema.ts`, `convex/stripe.ts`, `convex/_audit.ts`, `convex/audit/fixtures.ts`, `tests/audit/` |
| E-155 | 2026-09-05 | INT-AUD H5 | La agenda de turnos permitía que dos compradores tomaran el MISMO horario (AGD-01/02): `FormFillScreen.tsx:92-105` generaba la grilla en el cliente y `businessForms.submitLead:82` guardaba `scheduledDate`/`scheduledTime` como strings libres, sin mirar `businessSettings` ni chequear colisión. Un cliente modificado agendaba a las 3 AM de un domingo o en una fecha pasada. Además: cero manejo de zonas horarias en todo el repo, así que los horarios se interpretaban en la zona del dispositivo del comprador | La auditoría E-149 concluyó que "la agenda no está construida, es una feature ausente" porque el scanner buscaba escrituras en `bookings` (que sí está muerta). La agenda existía sobre otro sustrato: `businessFormLeads`, una tabla de contacto sin orden ni estados de cita. Misma causa raíz que STK-01: el chequeo vivía donde no podía escribir | `convex/_agenda.ts` puro (grilla, conversión de reloj de pared a instante con `Intl` respetando DST, ventana de cancelación) con 28 tests — incluidos la hora que NO EXISTE al adelantar el reloj (devuelve `null` y se saltea) y la que ocurre DOS VECES al atrasarlo (resuelve determinista). Tabla `appointments` con `by_business_and_start` como clave de colisión e `internalReserveAppointmentSlot` que chequea y escribe en una sola mutation, calcado de `internalReserveStock` (H3). `getAvailableSlots` pasa a ser la única autoridad; `FormFillScreen` y `MyBookingsScreen` dejan de calcular grillas. Modo por negocio (pago/solicitud) y cancelación a 24 h configurable, con `force` de admin + `audit_logs APPOINTMENT_REFUND_FORCED` — tercera pieza del mismo criterio que BON-07 (H2) y las entradas (H4). `businessFormLeads` deja de ser fuente de verdad paralela. **Arregla un agujero que no era de esta auditoría**: `internalAutoReleaseServices` esperaba un `delivered` que nadie escribe, así que el escrow de servicios no se liberaba nunca; ahora libera 24 h después de que el turno terminó. **Tres bugs adyacentes**: el `try/catch` de `submitLead:144-146` se tragaba todos los errores salvo uno por su mensaje (anulaba el guard de auto-agendarse y convertía una sesión inválida en lead anónimo), dos full scans sin índice, y `MyBookingsScreen:47` mapeaba domingo a `7` mientras `AgendaConfigTab` lo guarda como `0`. **Defecto propio corregido antes de commitear**: reprogramar al mismo horario chocaba contra la reserva del propio comprador | ✅ Resuelto. 466/466 tests (28 nuevos), typecheck 0 errores. Tabla `bookings` borrada (0 escritores/lectores, forma de alquiler). 3 suites de integración nuevas pendientes de correr contra `ramgos-audit` | `convex/_agenda.ts`, `convex/agenda.ts`, `convex/schema.ts`, `convex/stripe.ts`, `convex/businessForms.ts`, `convex/businessSettings.ts`, `convex/events.ts`, `src/screens/FormFillScreen.tsx`, `src/screens/MyBookingsScreen.tsx`, `src/components/dashboard/AgendaConfigTab.tsx`, `tests/audit/` |
