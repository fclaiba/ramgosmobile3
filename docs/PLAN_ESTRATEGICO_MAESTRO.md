...
| **CLI-STRIPE** | Requerimientos del cliente — Parte 2 completa (comisión 10%, escrow, devoluciones, liquidaciones, Connect, panel financiero, QA en Test Mode) | 🟡 Código integrado, sin desplegar | 70% | El branch `origin/claude/citas-perfil-comercial-2a3t23` (Stripe Connect bi-modal completo: escrow sin fallback mock, payout a influencers, refunds automáticos, tests) se mergeó sin conflictos en `integrate/stripe-connect-rewrite` (E-136) — resuelve 6 de los 8 gaps de `docs/ARQUITECTURA_STRIPE_CONNECT_SPLIT.md`. Pendiente: cargar env vars nuevas en Convex/Stripe Dashboard, correr la migración del cursor de reconciliación, merge a `main`, y QA con Stripe test mode real (checkout → webhook → escrow release → payout) | 2026-09-02 |
