...
| **INT-AUD** | Integridad transaccional — auditoría (STK/AGD/PAY/BON/TRV), remediación H0-H4 | 🟡 H0-H4 cerrados | 75% | H0 ✅ deployment + tests reales. H1 ✅ seeds sin auth → internal; **hallazgo nuevo sin resolver**: escalación de privilegios en `syncUser` (`convex/users.ts:702`, crea cuenta con `role` arbitrario) — **hoy es el único bloqueante de apertura al público**, de otra categoría que STK/AGD/PAY/BON/TRV y sin hito propio todavía. H2 ✅ BON-07 (bono cancela en el refund). H3 ✅ STK-01/STK-03 (reserva atómica de stock antes de cobrar, `convex/stock.ts` + tabla `stockReservations`). H4 ✅ **AGD-06/STK-04 a nivel 4**: `internalIssueEventReservationsForOrder` emite QR por unidad desde el webhook (mismo trigger que los bonos); refund con el mismo criterio que BON-07 (`checked_in` bloquea sin `force`, `confirmed` cancela); restock de stock extendido a `event`. Investigar el hallazgo mostró que `eventCapacity`/`eventSoldCount` no eran una fuente de aforo paralela sin conectar — es schema sin escritor, ninguna pantalla los llena — así que se documentaron como aspiracionales en vez de forzar una unificación sin sentido. **"H0-H7" de sesiones previas no era un plan con 8 hitos definidos**: sólo H0-H4 tienen alcance concreto; "H5 agenda de turnos" es una feature entera (slots, solapamiento, timezone, cancelación) que hoy no existe en el producto, no una remediación — decisión de scope pendiente del usuario antes de tocarla. Falta re-correr `npm run test:audit`/`test:audit` de eventos contra `ramgos-audit` (deploy del usuario) | 2026-09-05 |
