...
content = content.replace(/ {4}\/\/ Helper to satisfy strict SessionRecord type\s+const createSessionMock[^}]+}\);\s+/g, '');

// 2. Fix the initial useEffect to not use _mockUser
content = content.replace(
/ {4}\/\/ 1\. Initialize Session \(MOCKED\)[\s\S]+? {4}\}, \[\]\);/,
...
content = content.replace(
/ {12}const session = createSessionMock\(user\.id\);\s+await storage\.setItem\(CURRENT_SESSION_KEY, JSON\.stringify\(\{ \.\.\.session, _mockUser: user \}\)\);\s+setState\(prev => \(\{[\s\S]+?\}\)\);/,
...
content = content.replace(
/ {12}const session = createSessionMock\(userId\);\s+await storage\.setItem\(CURRENT_SESSION_KEY, JSON\.stringify\(session\)\);\s+const user: PublicUser = \{[\s\S]+?\}\);\s+setState\(prev => \(\{[\s\S]+?\}\)\);/,
`            await storage.setItem(CURRENT_SESSION_KEY, userId);
...
content = content.replace(
/ {16}if \(prev\.session\) \{\s+storage\.setItem\(CURRENT_SESSION_KEY, JSON\.stringify\(\{ \.\.\.prev\.session, _mockUser: updatedUser \}\)\)\.catch\(console\.error\);\s+\}/g,
``
);
