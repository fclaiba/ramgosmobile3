...
def analyze_security(codebase: Codebase) -> List[Finding]:
    ...
    secret_patterns = [
        (r'(?:password|passwd|pwd)\s*=\s*["\'][^"\']{4,}["\']', "Password hardcodeado"),
        (r'(?:secret|api_?key|apikey|token)\s*=\s*["\'][^"\']{8,}["\']', "Secret/Key hardcodeado"),
...
def analyze_architecture(codebase: Codebase) -> List[Finding]:
    ...
    schema_content = codebase.get_content("convex/schema.ts") or codebase.get_content("convex/schema.js")
    if schema_content:
