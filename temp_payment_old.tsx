...
export default function PaymentScreen({ route, navigation }: any) {
    ...
    const sellerName: string = params.sellerName ?? 'Taquer├¡a El Sabor';
    ...
    const handleApplyPoints = () => {
        ...
        if (isNaN(pointsToUse) || pointsToUse <= 0) {
            show('Ingresa una cantidad v├ílida de puntos (m├¡nimo 1)', 'error');
            return;
        }
        ...
        if (discountValue > amount) {
            ...
            if (pointsToUse > maxPoints) {
                show(`Descuento ajustado al m├íximo: $${amount.toFixed(2)} (${maxPoints} pts)`, 'info');
    ...
    const handlePayment = async () => {
        ...
        if (requiresCard && cleanedCardNumber.length < 12) {
            show('Revisa que el n├║mero de tarjeta sea correcto.', 'warning');
            return;
        }

        const [expMonthRaw, expYearRaw] = expiry.split('/');
        if (requiresCard && (!expMonthRaw || !expYearRaw)) {
            show('Ingresa una fecha de expiraci├│n en formato MM/YY.', 'warning');
            return;
        }
        ...
        try {
            if (finalAmount > 0) {
                if (!stripeCreatePaymentIntentRef) {
                    show('Stripe no est├í inicializado. Configura y redeploya credenciales antes de cobrar.', 'error');
                    setLoading(false);
                    return;
                }
                ...
                // Fully paid with points ÔÇö no Stripe charge needed
        ...
        try {
            ...
            const receipt: PaymentRecord = {
                ...
                method: {
                    ...
                    last4: finalAmount > 0 ? cardNumber.replace(/\D/g, '').slice(-4).padStart(4, 'ÔÇó') : '0000',
                ...
                <ScrollView contentContainerStyle={styles.successScroll}>
                    <View style={styles.successContent}>
                        ...
                        >
                            ...
                            <Text style={styles.successTitle}>┬íPago Exitoso!</Text>
                            <Text style={styles.successSubtitle}>
                                Tu compra fue confirmada v├¡a {receiptProviderName}.
                            </Text>
                        </Animated.View>
                        ...
                        <Animated.View entering={FadeInDown.delay(200).springify()} style={{ width: '100%', alignItems: 'center' }}>
                            ...
                            {createdOrders.length > 0 && (
                                <View style={styles.orderSummary}>
                                    <Text style={styles.orderSummaryTitle}>├ôrdenes generadas</Text>
                        ...
                        <Animated.View entering={FadeInDown.delay(200).springify()} style={{ width: '100%', alignItems: 'center' }}>
                            ...
                            {createdOrders.length > 0 && (
                                <View style={styles.orderSummary}>
                                    ...
                                    <Text style={styles.orderSummaryHint}>Los pagos permanecer├ín en escrow por 10 d├¡as.</Text>
                                </View>
                            )}
                        </Animated.View>
                    </View>
                ...
                <ScrollView contentContainerStyle={styles.scrollContent} showsVerticalScrollIndicator={false}>
                    ...
                    <View style={styles.summaryContainer}>
                        <View style={styles.summaryHeader}>
                            ...
                            <Text style={styles.summaryMeta}>{itemCount} ├¡tems</Text>
                        </View>
                        ...
                        <View style={[styles.summaryRow, { marginTop: 6 }]}>
                            <Text style={[styles.summaryLabel, { color: '#F59E0B' }]}>Gan├ís en puntos</Text>
                        ...
                        <Text style={styles.summaryShipping}>
                            {shippingMethod === 'pickup'
                                ...
                                : `Env├¡o ${shippingLabel} ┬À ${shippingDestination?.city ?? 'Destino'}`}
                        </Text>
                    </View>
                        ...
                        <View style={styles.providerContainer}>
                                            ...
                                            <View style={{ flex: 1 }}>
                                                ...
                                                <Text style={styles.providerCaption}>
                                                    {provider.supportsSplit
                                                        ? 'Split autom├ítico y conciliaci├│n instant├ínea'
                                                        : 'Procesamiento est├índar'}
                                                </Text>
                                            </View>
                            ...
                            {selectedProviderDef && (
                                <Text style={styles.providerHelper}>
                                    Procesado por {selectedProviderDef.displayName} ÔÇó fondos seguros en custodia Ramgos.
                                </Text>
                            )}
                        </View>
                    )}
                    ...
                    <View style={styles.cardVisualContainer}>
                        ...
                        >
                            ...
                            <Text style={styles.cardNumberVisual}>{cardNumber || 'ÔÇóÔÇóÔÇóÔÇó ÔÇóÔÇóÔÇóÔÇó ÔÇóÔÇóÔÇóÔÇó ÔÇóÔÇóÔÇóÔÇó'}</Text>
                    ...
                    <View style={styles.securityBadge}>
                        ...
                        <Text style={styles.securityText}>Tus datos est├ín protegidos con encriptaci├│n SSL.</Text>
                    </View>
